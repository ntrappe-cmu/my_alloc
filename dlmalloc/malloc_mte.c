...
// NO MORECORE!! Otherwise MTE not backed
#ifdef MTE_ENABLED
#undef MORECORE
#define MORECORE(size) ((void*)MORECORE_FAILURE)
#endif
...
#endif
#define MMAP_PROT   (PROT_READ|PROT_WRITE|PROT_MTE)
#else
...
/* ----------- Helper routines dealing with MTE support -------------- */
...
/**
 * Create a random tag for the chunk of memory that will be handed back to
 * the user. Then stamp each granule with the tag until we hit the end of
 * the chunk. We use the size of  the ACTUAL chunk size in bytes, not the 
 * amount of memory the user asked from us. Includes header which is why
 * we need to strip this tag on every access to jump b/w chunks.
 */
static Void_t *mte_tag_chunk(mchunkptr p) {
...
/**
 * Identify the tag for this chunk of memory so we can strip it for accesses.
 */
static mchunkptr mte_get_chunk_tag(mchunkptr p) {
  return (mchunkptr)__arm_mte_get_tag((void *)p);
}

/**
 * Ptr MUST ALREADY BE UNTAGGED. Overwrite tag with 0 one over each granule
 * so we can access header and other info.
 * Unlike tagging, we do not calc size inside function because UNTAG returns
...
#define set_inuse_bit_at_offset(p, s) {                          \
  mchunkptr _next = (mchunkptr)(((char*)(p)) + (s));             \
  _next = mte_get_chunk_tag(_next);                              \
  _next->size |= PREV_INUSE;                                     \
}
#else
#define set_inuse_bit_at_offset(p, s)\
...
#define inuse_bit_at_offset(p, s) ({                             \
  mchunkptr _nb = (mchunkptr)(((char*)(p)) + (s));               \
  _nb = mte_get_chunk_tag(_nb);                                  \
  (_nb->size & PREV_INUSE);                                      \
})
#else
#define inuse_bit_at_offset(p, s)\
...
#ifdef MTE_ENABLED
#define set_foot(p, s) {                                         \
  mchunkptr _foot = (mchunkptr)((char*)(p) + (s));               \
  _foot = mte_get_chunk_tag(_foot);                              \
  _foot->prev_size = (s);                                        \
}
#else
...
void print_av(mstate av) {
  if (!av) return;
  const int fastbin_preview_limit = 6;
  printf("======================== START AV ========================\n");
  printf("Top: %p\n", (void *)av->top);
  printf("Binmap:|");
  for (int i = 0; i < BINMAPSIZE; i++) {
    printf("%d|", av->binmap[i]);
  }
  printf("\n");
  printf("Fastbins:\n");
  for (int i = 0; i < NFASTBINS; ++i) {
    mchunkptr cur = av->fastbins[i];
    int depth = 0;
    if (cur == 0) {
      printf("  [%02d] NULL\n", i);
      continue;
    ...
    printf("  [%02d] head=%p", i, (void*)cur);
    while (cur != 0 && depth < fastbin_preview_limit) {
      printf(" -> %p(sz=0x%lx)", (void*)cur, (unsigned long)chunksize(cur));
      cur = cur->fd;
      depth++;
    }
    if (cur != 0) {
...
#endif
{
  ...
  */
  ...
  if ((CHUNK_SIZE_T)(nb) >= (CHUNK_SIZE_T)(av->mmap_threshold) &&
    ...
    */
    ...
    // printf("@sysalloc, finding a slightly larger fastbin %ld\n", size);

    /* Don't try if size wraps around 0 */
    if ((CHUNK_SIZE_T)(size) > (CHUNK_SIZE_T)(nb)) {

      mm = (char*)(MMAP(0, size, MMAP_PROT, MAP_PRIVATE));
      // printf("@sysmalloc, use mmap to allocate size %d\n", (int)size);
  ...
  */
  ...
  // printf("@sysmalloc, requesting space %d bytes in chunk\n", (int)size);
  ...
  */
  ...
  if (brk == (char*)(MORECORE_FAILURE)) {
    ...
    if ((CHUNK_SIZE_T)(size) > (CHUNK_SIZE_T)(nb)) {
      ...
      brk = (char*)(MMAP(0, size, MMAP_PROT, MAP_PRIVATE));
  ...
  if (brk != (char*)(MORECORE_FAILURE)) {
    ...
    */
    ...
    if (brk == old_end && snd_brk == (char*)(MORECORE_FAILURE)) {
      // printf("@sysmalloc & mmap, extend top size\n");
...
#endif
{
  ...
  */
  ...
  if (!have_anychunks(av)) {
    if (av->max_fast == 0) /* initialization check */
      // printf("@dlmalloc, no frees yet so go make memory\n");
      malloc_consolidate(av);
    goto use_top;
  }
  ...
  */
    ...
    // printf("@dlmalloc, we are largebin request\n");
    if (have_fastchunks(av)) 
      malloc_consolidate(av);
  }
...
#endif
{
  ...
  if (mem != 0) {
    ...
    */
      ...
      // Need to get the next chunk's tag to access
      #ifdef MTE_ENABLED
